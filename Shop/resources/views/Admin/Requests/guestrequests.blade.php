                ...
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        ...
                        <div class="filter-group">
                            <label>ФИО</label>
                            <input id="guest-name" type="text" class="form-control">
                        </div>
                ...
                <tbody id="orders-container">
                    @foreach ($guestRequests as $guestRequest)
                        ...
                        <tr  id="orders-contentT" >
                            <td id="order-row">{{$guestRequest->id}}</td>
                            <td>{{$guestRequest->fullname}}</td>
                            <td>{{$guestRequest->country}}</td>
                            <td>{{$guestRequest->town}}</td>
                            <td>{{$guestRequest->phone}}</td>
                            <td>{{$guestRequest->email}}</td>
                            <td>{{date('Y-m-d H:i:s', ($guestRequest->created_at))}}</td>                        
                            <td> {{ $requestTypes->firstWhere('id', $guestRequest->request_id)->title }} </a></td>
                           
                        
                            <td><a href="{{ route('guestrequests.edit', [ 'id' => $guestRequest->id ]) }}"
...
$(document).ready(function() {
    ...
    let search_string = $('#guest-name').val()
    // Функция для обновления содержимого таблицы
    function updateTable() {
        $.ajax({
            url: '{{ route("guestrequests.index") }}',
            type: 'GET',
            data: {
    ...
    $(document).on('keyup', '#guest-name', function(e) {
        e.preventDefault();
...
//     // Обработчик события клика по ссылке пагинации
    ...
    function product(page) {
        var perPage = $('#entries-select').val();
        $.ajax({
            url: '{{ route("guestrequests.index") }}',// '/admin/home/orders',
            type: 'GET',
            data: {
                page: page,
                perPage: perPage,
